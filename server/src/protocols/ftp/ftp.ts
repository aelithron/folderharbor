...
export default async function startFTP(port: number, sslKey?: string, sslCert?: string): Promise<FtpSrv> {
  ...
  if (sslKey && sslCert) {
    server = new FtpSrv({ url: "ftps://0.0.0.0:" + port, anonymous: false, tls: { key: sslKey, cert: sslCert }, log: logger, blacklist: ["PORT"] });
  } else {
    server = new FtpSrv({ url: "ftp://0.0.0.0:" + port, anonymous: false, log: logger, blacklist: ["PORT"] });
  }
