...
export async function setUpServer() {
  ...
  await writeLog(0, null, "acls-create", { id: acl.id, newContents: { name: "Everything", allow: ["**/*"] }, protocol: "internal" }, "created an ACL (setup script)");
  ...
  await writeLog(0, null, "roles-create", { id: role.id, newContents: { name: "Administrator", acls: [acl.id], permissions: getPermissionIDs() }, protocol: "internal" }, "created a role (setup script)");
  const user = await createUser(adminUser, adminPass, { roles: [role.id] });
  if ("error" in user) {
    console.error(`Error creating user: ${user.error}`);
    process.exit(1);
  }
  await writeLog(0, null, "users-create", { id: user.id, newContents: { username: adminUser, password: "[redacted]", roles: [role.id] }, protocol: "internal" }, "created a user (setup script)");
...
export async function recoverServer() {
  ...
  await writeLog(0, null, "acls-create", { id: acl.id, newContents: { name: "Everything", allow: ["**/*"] }, protocol: "internal" }, "created an ACL (recovery script)");
  const role = await createRole("Administrator", { acls: [acl.id], permissions: getPermissionIDs() });
  if ("error" in role) {
    console.error(`Error creating role: ${role.error}`);
    process.exit(1);
  }
  await writeLog(0, null, "roles-create", { id: role.id, newContents: { name: "Administrator", acls: [acl.id], permissions: getPermissionIDs() }, protocol: "internal" }, "created a role (recovery script)");
  const user = await createUser(adminUser, adminPass, { roles: [role.id] });
  if ("error" in user) {
    console.error(`Error creating user: ${user.error}`);
    process.exit(1);
  }
  await writeLog(0, null, "users-create", { id: user.id, newContents: { username: adminUser, password: "[redacted]", roles: [role.id] }, protocol: "internal" }, "created a user (recovery script)");
  ...
  for (const oldUser of users) {
    ...
    await writeLog(user.id, adminUser, "users-edit", { id: oldUser.id, newContents: { locked: true }, protocol: "internal" }, "edited a user (recovery script)");
  }
