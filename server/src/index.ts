...
async function startServer() {
  ...
  if ((process.getuid && process.getgid) && (process.getuid() === 0 || process.getgid() === 0) && !program.opts().allowRootUser) {
    console.error('You started FolderHarbor as root! This is a security risk.\nIf you understand the risks, you can re-invoke the server with the "--allow-root-user" argument.');
    process.exit(1);
  }
  console.log(`Starting FolderHarbor!`);
  config = await getConfig((program.opts().config as string | undefined) ? path.resolve(program.opts().config as string) : undefined);
  if (config.database === "") throw new Error("Missing or invalid database credentials!");
}
await startServer();